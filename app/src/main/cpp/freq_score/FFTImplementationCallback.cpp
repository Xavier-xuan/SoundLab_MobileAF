...
namespace internal {
namespace fft {
void FFTImplementationCallback::doHalfLengthRadix2(const double x[256],
                                                   creal_T y[256])
{
  ...
  y[128].re = 0.5 * ((im - temp_im_tmp) + (im - (-re)));
  y[128].im = 0.5 * ((im + temp_im_tmp) + (-im + re));
  for (i = 0; i < 63; i++) {
    ...
    y[i + 129].re = 0.5 * ((temp_re_tmp * temp_im - temp_im_tmp * b_im) +
                           (temp2_re * b_re - -temp2_im * temp_re));
    y[i + 129].im = 0.5 * ((temp_re_tmp * b_im + temp_im_tmp * temp_im) +
    ...
    y[istart + 127].re = 0.5 * ((temp2_re * re - temp2_im * im) +
                                (temp_re_tmp * temp_im - -temp_im_tmp * b_im));
    y[istart + 127].im = 0.5 * ((temp2_re * im + temp2_im * re) +
  ...
  re = -temp_im_tmp * 0.0;
  y[64].re = 0.5 * ((temp_re - b_re) + (im * 2.0 - re));
  y[64].im = 0.5 * ((temp_re + b_re) + (im * 0.0 + -temp_im_tmp * 2.0));
  y[192].re = 0.5 * ((im * 2.0 - temp_im_tmp * 0.0) + (im * 0.0 - re));
  y[192].im = 0.5 * ((im * 0.0 + temp_im_tmp * 2.0) + (im * 0.0 + re));
}
                      ...
                      const double in2[256], creal_T in3[256])
{
  ...
  for (int i{0}; i < 256; i++) {
    b_in1[i] = in1[i * stride_0_0] * in2[i];
  }
  coder::internal::fft::FFTImplementationCallback::doHalfLengthRadix2(b_in1,
                                                                      in3);
}

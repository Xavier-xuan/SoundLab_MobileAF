                    ...
                    const mxArray *plhs[2])
{
  ...
  real_T(*total_socres_data)[4];
  real_T final_score;
  real_T fs;
  int32_T total_socres_size[2];
  st.tls = emlrtRootTLSGlobal;
  total_socres_data = (real_T(*)[4])mxMalloc(sizeof(real_T[4]));
  emlrtHeapReferenceStackEnterFcnR2012b(&st);
  // Marshall function inputs
  signals.no_free();
  emlrt_marshallIn(st, emlrtAlias(prhs[0]), "signals", signals);
  fs = emlrt_marshallIn(st, emlrtAliasP(prhs[1]), "fs");
  // Invoke the target function
  freq_score(signals, fs, &final_score, *total_socres_data, total_socres_size);
  // Marshall function outputs
  plhs[0] = emlrt_marshallOut(final_score);
  if (nlhs > 1) {
    plhs[1] = emlrt_marshallOut(*total_socres_data, total_socres_size);
  }
...
void freq_score_atexit()
{
  ...
  freq_score_xil_terminate();
  freq_score_xil_shutdown();
...
// End of code generation (_coder_freq_score_api.cpp)
